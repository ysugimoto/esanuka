...
const validateApiKeys = async apiKeys => {
  const errors = [];
  apiKeys.forEach((key, i) => {
    if (!key.name) {
      errors.push(`api key name is required in ${i}`);
    }
    if (!key.value) {
      errors.push(`api key value is required in ${i}`);
    }
  });
  if (errors.length > 0) {
    throw new Error(
    `Invalid ApiKey definitions found: ${errors.join('\n')}`,
    );
  }
};
...
const validateLambdaIntegration = async (path, httpMethod, method) => {
  ...
  try {
    ...
    console.log(`[SKIPPED] Function ${method.function} is not found, ensure exists on deploy.`)
    return;
  }
...
const validateDefinition = async (restApiId, defs) => {
  ...
  // validate api keys
  try {
    await validateApiKeys(defs.apiKeys || []);
  } catch (err) {
    errors.push(err);
  }
