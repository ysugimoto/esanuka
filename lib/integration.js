...
const addLambdaFunctionPermission = async (resourceBase, path, functionArn, functionName, alias = '') => {
  ...
  try {
    ...
    if (err.code !== 'ResourceConflictException') {
      logger(`Failed to add lambda permission: ${JSON.stringify(input, null, "  ")}, message: ${err.message}`);
      throw new Error(err.message);
    }
    logger(`Lambda function permission already exists: ${input.FunctionName}, statementId: ${input.StatementId}`);
    return;
  }
  logger(`Added Lambda function permission for ${input.FunctionName}, statementId: ${input.StatementId}`);
};
